...
app.config['SESSION_REDIS'] = redis.from_url(
    "redis://default:BYmFlNJIAmXSJl6kZWgbg6CVFKS4Af2u@redis-16143.c57.us-east-1-4.ec2.redns.redis-cloud.com:16143"
)
...
app.config['SECRET_KEY'] = 'super-secret-key'  # You can change this
...
class CustomOAuth(OAuth):
    ...
    def create_client(self, name):
        ...
        client.state_storage = RedisStateStorage(
            redis.from_url(
                "redis://default:BYmFlNJIAmXSJl6kZWgbg6CVFKS4Af2u@redis-16143.c57.us-east-1-4.ec2.redns.redis-cloud.com:16143"
            ))
        return client
...
google = oauth.register(
    name='google',
    client_id=
    "73238950328-eou55q72gn2cg89ei20rv7mhdb03bs8v.apps.googleusercontent.com",
    client_secret="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",
    server_metadata_url=
...
CG_API_KEY = "CG-5iEFdph6Ch6t5AeDw7xaTh83"
...
def authorize():
    ...
    if not user:
        cursor.execute(
            "INSERT INTO users (id, name, username, email, picture, joined_at, credits) VALUES (%s, %s, %s, %s, %s, %s, %s)",
    ...
    return redirect("/")  # ✅ ✅ ✅ internal home redirect
...
def get_profile():
    ...
    print("HEADERS DEBUG:", dict(request.headers))
    print("SESSION DEBUG:", dict(session))
    if 'user' not in session:
        return jsonify({"error": "Not logged in"}), 401
...
def place_bet():
    ...
    if not all([user_id, round_id, crypto, direction
                ]) or direction not in ("up", "down") or amount <= 0:
...
def get_round_result():
    ...
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT end_price, result FROM rounds WHERE id = %s",
                   (round_id, ))
    round_data = cursor.fetchone()
    cursor.close()
    release_conn(conn)
...
def manage_stock_rounds(price_data):
    ...
    for symbol, details in price_data.items():
        ...
        if not existing:
            ...
            print(
                f"✅ Created new STOCK round for {symbol} starting at ${price}")
            conn.commit()
...
def run_stock_close_resolution():
    print("📉 Resolving stock rounds at market close...")
    stock_prices = get_all_stock_prices()
    conn = get_conn()
    cursor = conn.cursor()

    for symbol, end_price in stock_prices.items():
        # Get today's open round
        ...
        if round_data:
            ...
            print(
                f"✅ Resolved {symbol} round ID {round_data['id']} as {result}")
...
TWELVE_API_KEY = "36ea376783924b6184c3b9569c7e1c21"
...
def resolve_round(round_id, result):
    ...
    print(f"🔍 Resolving round {round_id} with result: {result}")
    ...
    if result == "same":
        for bet in all_bets:
            ...
            cursor.execute(
                """
                UPDATE bets 
                SET status = 'refunded', profit = 0 
                WHERE id = %s
            """, (bet['id'], ))
        print(f"🔁 Round {round_id} refunded due to same price.")
    else:
        for bet in all_bets:
            if bet['direction'] == result:
                ...
                cursor.execute(
                    """
                    UPDATE bets 
                    SET status = 'won', profit = %s 
                    WHERE id = %s
                """, (profit, bet['id']))
                print(f"✅ {bet['user_id']} won {winnings} credits!")
            else:
                loss = -bet['amount']
                cursor.execute(
                    """
                    UPDATE bets 
                    SET status = 'lost', profit = %s 
                    WHERE id = %s
                """, (loss, bet['id']))
                print(f"❌ {bet['user_id']} lost.")
...
def manage_round_for_symbol(symbol, price_data):
    ...
    if coin_id not in price_data:
        print(f"⚠️ Missing price for {symbol} from CoinGecko.")
        return
    ...
    if last_round and current_time > last_round["end_time"] and last_round[
            "result"] is None:
        result = "same"
        if price > last_round["start_price"]:
            result = "up"
        elif price < last_round["start_price"]:
            result = "down"

        cursor.execute(
            "UPDATE rounds SET end_price = %s, result = %s WHERE id = %s",
            (price, result, last_round["id"]))
        conn.commit()
        resolve_round(last_round["id"], result)
        print(f"✅ Round {last_round['id']} for {symbol} resolved: {result}")
    ...
    if not last_round or current_time > last_round["end_time"]:
        cursor.execute(
            ...
            (symbol, price, current_time, current_time + 120))
        conn.commit()
        print(f"🆕 New round created for {symbol} at price ${price}")
...
def run_rounds_forever():
    while True:
        ...
        for symbol in CRYPTO_MAP.keys():
            manage_round_for_symbol(symbol, price_data)
        time.sleep(5)
